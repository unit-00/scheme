...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in curr.bindings:
            ...
            return curr.parent.lookup(symbol)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*list_args), None
        except TypeError:
            raise SchemeError('Something went wrong')
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            proc = scheme_eval(self.body,frame)
            return proc, None
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    body = vals.second
    if len(body) == 0:
        raise SchemeError('Expected at least one argument')
    elif len(body) == 1:
        body = body.first
    else:
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        func_args = target.second
        func_body = vals.second
        if scheme_numberp(func_name):
            raise SchemeError('Function names cannot be a number')
        lambda_proc, e = do_lambda_form(Pair(func_args, func_body), env)
...
def do_let_form(vals, env):
    ...
    for b in bindings:
        vals = scheme_eval(b[1], env)
        new_env.define(b.first, vals)
...
def do_if_form(vals, env):
    ...
    # exp = scheme_eval(vals.second.first, env)
    cond = vals.first
    exp = vals.second
    if scheme_eval(cond, env):
        return scheme_eval(exp.first, env), None
    else:
        if len(exp) == 1:
            return okay, None
        else:
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            "*** YOUR CODE HERE ***"
            if len(clause.second):
                body = Pair('begin', clause.second)
...
def check_formals(formals):
    ...
    for p in formals:
        if p in already_seen:
            raise SchemeError('Argument is repeated')
        if not scheme_symbolp(p):
            raise SchemeError('Not a well-formd list of symbols')
